  ...
  query getProducts($presc_ids: [ID!]!) {
    nodes(ids: $presc_ids) {
      ... on Product {
        ...
        descriptionHtml
        id
        sg_sv_std_tc: metafield(namespace: "sg_sv_std_tc", key: "sg_sv_std_tc") {
          id
          key
          namespace
          value
...
const MultiStandardInputsSunTransclr = () => {
        ...
        {(handleSubmit, {error, data}) => {
          ...
          return (
            <Frame>
              ...
              <Page title="Standard Transition/Clear (Non-mirrored)" description="desc">
                          ...
                          onAction: () => {
                            if(inputFields[0].image_url != ''){
                                ...
                                for (var k = 0; k < intersection.length; k++) {
                                  const intersectionId =intersection[k];
                                  var productmeta = {
                                    id: intersectionId,
                                    "metafields": metadat
                                  }
                              ...
                              axios.post("/admin/file",formData,config)
                                .then((response) => {
                                    ...
                                    const firstArray = store.get('multi_presc_ids');
                                    const secondArray = store.get('multi_ids');
                                    var intersection = firstArray.filter(element => secondArray.includes(element));
                                    
                                    
                                    for (var k = 0; k < intersection.length; k++) {
                                      var intersectionId =intersection[k];
                                      var productmeta = {
                                        id: intersectionId,
                                        "metafields": metadat
                                      }
...
export default MultiStandardInputsSunTransclr;
