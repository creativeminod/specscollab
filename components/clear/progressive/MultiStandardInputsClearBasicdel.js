  ...
  query getProducts($presc_ids: [ID!]!) {
    nodes(ids: $presc_ids) {
      ... on Product {
        ...
        descriptionHtml
        id
        clr_pr_clr: metafields(namespace: "clr_pr_clr", first: 100){
          edges {
            node {
              id
              namespace
                      ...
                      <FormLayout.Group>
                        <Query query={GET_PRODUCTS_BY_ID} variables={{ presc_ids: store.get('presc_ids') }}>
                                {({ data, loading, error }) => {
                                  ...
                                  if (data.nodes[0].clr_pr_clr){
                                    const clr = data.nodes[0].clr_pr_clr.edges.length;
                                    let clrarray = [];
                                    for (let i = 0; i < clr; i++) {
                                      const imag = data.nodes[0].clr_pr_clr.edges[i].node.value.split('"');
                                      const idx = data.nodes[0].clr_pr_clr.edges[i].node.id;
                                      const idx_key = data.nodes[0].clr_pr_clr.edges[i].node.key;
                                      const idx_namespace = data.nodes[0].clr_pr_clr.edges[i].node.namespace;
                                              ...
                                              primaryAction={[
                                                {
                                                  ...
                                                  onAction: () => {
                                                      const productmetadel = {                                                  
                                                          "id": data.nodes[0].clr_pr_clr.edges[i].node.id
                                                      }
                                                      handleSubmit({
                                                        variables: { 
                                              ...
                                              secondaryActions={[
                                                {
                                                  content: 'Edit',
                                                  onAction: () => {
                                                    store.set('lens_edit-id', data.nodes[0].clr_pr_clr.edges[i].node.id);
                                                    store.set('lens_edit-key', data.nodes[0].clr_pr_clr.edges[i].node.key);
                                                    store.set('lens_edit-namespace', data.nodes[0].clr_pr_clr.edges[i].node.namespace);
