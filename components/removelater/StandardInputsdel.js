  ...
  query getProducts($presc_ids: [ID!]!) {
    nodes(ids: $presc_ids) {
      ... on Product {
        ...
        standard_lenses: metafields(namespace: "standard_lenses", first: 10){
          edges {
            node {
              id
              namespace
                            ...
                            <FormLayout.Group>
                              <Query query={GET_PRODUCTS_BY_ID} variables={{ presc_ids: store.get('presc_ids') }}>
                                {({ data, loading, error }) => {
                                  ...
                                  if(data.nodes[0].standard_lenses){
                                    const clr = data.nodes[0].standard_lenses.edges.length;
                                    let clrarray = [];
                                    for (let i = 0; i < clr; i++) {
                                      const imag = data.nodes[0].standard_lenses.edges[i].node.value.split('"');
                                      const idx = data.nodes[0].standard_lenses.edges[i].node.id;
                                      const idx_key = data.nodes[0].standard_lenses.edges[i].node.key;
                                            ...
                                            primaryAction={[
                                              {
                                                ...
                                                onAction: () => {
                                                    const productmetadel = {                                                  
                                                        "id": data.nodes[0].standard_lenses.edges[i].node.id
                                                    }
                                                    handleSubmit({
                                                      variables: { 
...
export default StandardInputsdel;
