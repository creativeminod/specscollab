...
const PrizmInputsNonpolarizeddel = () => {
                            ...
                            <FormLayout.Group>
                              <Query query={GET_PRODUCTS_BY_ID} variables={{ presc_ids: store.get('presc_ids') }}>
                                {({ data, loading, error }) => {
                                  ...
                                  if(data.nodes[0].prizm_nonpol){
                                    const clr = data.nodes[0].prizm_nonpol.edges.length;
                                    let clrarray = [];
                                    for (let i = 0; i < clr; i++) {
                                      const imag = data.nodes[0].prizm_nonpol.edges[i].node.value.split('"');
                                      const idx = data.nodes[0].prizm_nonpol.edges[i].node.id;
                                      const idx_key = data.nodes[0].prizm_nonpol.edges[i].node.key;
                                            ...
                                            primaryAction={[
                                              {
                                                ...
                                                onAction: () => {
                                                    const productmetadel = {                                                  
                                                        "id": data.nodes[0].prizm_nonpol.edges[i].node.id
                                                    }
                                                    handleSubmit({
                                                      variables: { 
