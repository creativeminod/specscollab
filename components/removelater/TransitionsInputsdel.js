                            ...
                            <FormLayout.Group>
                              <Query query={GET_PRODUCTS_BY_ID} variables={{ presc_ids: store.get('presc_ids') }}>
                                {({ data, loading, error }) => {
                                  ...
                                  if(data.nodes[0].transitions_lenses){
                                    const clr = data.nodes[0].transitions_lenses.edges.length;
                                    let clrarray = [];
                                    for (let i = 0; i < clr; i++) {
                                      const imag = data.nodes[0].transitions_lenses.edges[i].node.value.split('"');
                                          ...
                                          <PageActions primaryAction={[{
                                            ...
                                            onAction: () => {
                                                const productmetadel = {                                                  
                                                    "id": data.nodes[0].transitions_lenses.edges[i].node.id
                                                }
                                                handleSubmit({
                                                  variables: { 
...
export default TransitionsInputsdel;
