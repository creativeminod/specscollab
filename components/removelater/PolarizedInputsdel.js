  ...
  query getProducts($presc_ids: [ID!]!) {
    nodes(ids: $presc_ids) {
      ... on Product {
        ...
        polarized_lenses: metafields(namespace: "polarized_lenses", first: 10){
          edges {
            node {
              id
              namespace
                            ...
                            <FormLayout.Group>
                              <Query query={GET_PRODUCTS_BY_ID} variables={{ presc_ids: store.get('presc_ids') }}>
                                {({ data, loading, error }) => {
                                  ...
                                  if(data.nodes[0].polarized_lenses){
                                    const clr = data.nodes[0].polarized_lenses.edges.length;
                                    let clrarray = [];
                                    for (let i = 0; i < clr; i++) {
                                      const imag = data.nodes[0].polarized_lenses.edges[i].node.value.split('"');
                                      const idx = data.nodes[0].polarized_lenses.edges[i].node.id;
                                      const idx_key = data.nodes[0].polarized_lenses.edges[i].node.key;
                                        ...
                                        <PageActions primaryAction={[{
                                          ...
                                          onAction: () => {
                                              const productmetadel = {                                                  
                                                  "id": data.nodes[0].polarized_lenses.edges[i].node.id
                                              }
                                              handleSubmit({
                                                variables: { 
...
export default PolarizedInputsdel;
